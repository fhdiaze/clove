...
{
	const size_t n1 = L->n1;
	bool (*const restrict M)[n1] = (void *)L->Mv;
	const size_t off0 = L->off0;
...
static void border(size_t m, int b)
{
...
void life_draw(life *L)
{
	const size_t n1 = L->n1;
	bool (*const restrict M)[n1] = L->Mv;
	const size_t x0 = L->x0;
	const size_t x1 = L->x1;
	const size_t off0 = L->off0;
	const size_t off1 = L->off1;
	const size_t len0 = L->len0;
	const size_t len1 = L->len1;
...
static bool hashin(bool (*visited)[life_maxit], size_t hash)
{
	bool ret = false;
	hash %= life_maxit;
	if (!(*visited)[hash]) {
...
life *life_init(struct life *L, size_t n, size_t m, bool mat[static n][m])
{
	if (L) {
		L->n0 = n;
		L->n1 = m;
