...
#define static_assert_compatible(A, B, REASON)                                           \
	static_assert(_Generic((typeof(A) *)nullptr, typeof(B) *: true, default: false), \
...
#define minunsigned(X, Y) \
	(_Generic(tozero(X) + tozero(Y), typeof(X): tozero(Y), default: tozero(X)))

#define minreturn(X, Y)                                                                             \
	_Generic((char (*)[mincharacteristic(X, Y)]){ 0 }, /* both signed, arithmetic conversion */ \
	        char (*)[4 | 2 | 1]: tozero(X) + tozero(Y), /* only one signed, use it */           \
	        char (*)[0 | 2 | 1]: tozero(Y),                                                     \
	        char (*)[4 | 0 | 1]: tozero(X), /* both unsigned, use narrower */                   \
	...
	({                                                                                 \
		...
		((isnegative(min_x) && !issigned(min_y)) ?                                 \
		         ...
		         ((isnegative(min_y) && !issigned(min_x)) ?                        \
		                  ...
		                  positive */          \
		                   (min_x < min_y) ? (min_type)min_x : (min_type)min_y))); \
	})
