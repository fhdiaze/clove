...
[[deprecated("implementation")]]
static size_t circular_getpos(const circular c[static 1], size_t pos)
{
	if (c->cap) {
		pos += c->start;
		pos %= c->cap;
...
[[deprecated("implementation")]]
double *circular_element(const circular *c, size_t pos)
{
	double *ret = nullptr;
	if (c) {
		if (pos < c->cap) {
	...
	if (c) {
		...
		if (nlen != olen) {
			...
			if (nlen > olen) {
				...
				// non-empty, and there were already two
				// separate chunks
				if (ostart + len > olen) {
				...
				if (ostart + len > olen) {
					// Two separate chunks; mv the upper one
					// down to the new end.
					size_t ulen = olen - ostart;
					nstart = nlen - ulen;
				...
				if (nlen) {
					...
					// There had been no elements and we
					// were asked to reduce the buffer to 0
					// elements.
					free(c->tab);
					ntab = nullptr;
					nstart = 0;
