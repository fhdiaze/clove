...
time_t isotime(const char t[static 1])
{
	struct tm date[1] = {
            [0] =
                {
...
void time2terran(time_t t)
{
	/* Provide us with the terran epoch. This is not as easy as it may
	   ...
	   mktime and gmtime to obtain the correct timestamp at 1969-12-22
	   0:0:0.
	 */
	...
	*/
	...
	if (days < dq0) {
		...
		days -= dq0;
		/*
		   In the remaining period every fourth year starting with year
		   0 is a leap year. Compute the amount of such 4 year periods
		   and adjust
		*/
		const long qer = days / dq1;
		year += qer * yq;
		days %= dq1;
