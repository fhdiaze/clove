...
int main(int argc, char *argv[argc + 1])
{
		...
		if (!setlocale(LC_ALL, "")) perror("couldn't set locale");
		else perror("With default locale");
		if (!setlocale(LC_MESSAGES, "C")) perror("couldn't set locale");
		else perror("C locale, again");
		if (argv[1] && !setlocale(LC_MESSAGES, argv[1]))
				perror("couldn't set locale");
		else perror("new locale");
