...
#define GENERIC_IF(COND, A, B) _Generic((char (*)[1 + !!(COND)])0, char (*)[2]: (A), default: (B))
...
#define isdecimalfloating(...)       \
	(_Generic((__VA_ARGS__) + 0, \
	         _C23_Decimal32_CASE _C23_Decimal64_CASE _C23_Decimal128_CASE default: false))
#ifndef iscomplex
#define iscomplex(...)                                            \
	(_Generic((__VA_ARGS__) + 0,                              \
...
#define isstandardrealfloating(...) \
	(_Generic((__VA_ARGS__) + 0, float: true, double: true, long double: true, default: false))
...
#ifndef issigned
#define issigned(...)                                                             \
	((bool)(is_potentially_negative(__VA_ARGS__) && isinteger(__VA_ARGS__) && \
	        !iscompatible(__VA_ARGS__, char)))
#endif
#ifndef isunsigned
#define isunsigned(...)                                                            \
	((bool)(!is_potentially_negative(__VA_ARGS__) && isinteger(__VA_ARGS__) && \
...
#ifndef isxwide
#define isxwide(...)                      \
	((bool)(isinteger(__VA_ARGS__) && \
	        _Generic((__VA_ARGS__) + 0ULL, unsigned long long: false, default: true)))
#endif
...
#define get_fla(...) \
	GENERIC_IF(isvla(__VA_ARGS__), (struct do_not_use_this_otherwise[1]){ 0 }, (__VA_ARGS__))
...
#define bsearch(KEY, BASE, NMEMB, SIZE, COMPAR) \
	((typeof(1 ? (BASE) : (void *)1))bsearch((KEY), (BASE), (NMEMB), (SIZE), (COMPAR)))
#endif
...
#define shift_xright(...)                                                              \
	GENERIC_IF(isxwide(__VA_ARGS__),                                               \
	           (__VA_ARGS__) >> GENERIC_IF(isxwide(__VA_ARGS__), ULLONG_WIDTH, 0), \
	           tozero(__VA_ARGS__))
...
#define stdc_bit_floor(...)                                                              \
	({                                                                               \
		...
		(!s_b_f_x) ? 0 : (toone(s_b_f_x) << (stdc_bit_width(s_b_f_x) - 1));      \
	})
