...
size_t mbsrlen(const char *s, const mbstate_t *restrict state)
{
...
wchar_t *mbsrdup(const char *s, mbstate_t *restrict state)
{
...
size_t mbsspn(const char *s1, const char *s2)
{
...
size_t mbsspn_class(const char *s1, wcclass_t func)
{
...
size_t mbsspn_type(const char *s1, wctype_t type)
{
	return s1 ? mbsrskip_type(s1, 0, type) - s1 : 0;
}

size_t mbsspn_name(const char *s1, const char name[static 1])
{
...
size_t mbscspn(const char *s1, const char *s2)
{
	       ...
	       const mbstate_t *restrict sstate)
{
	...
	for (size_t slen = 0; s[0] && n; s += slen) {
		...
		/* Here, wcsrtombs can't fail, but because of the different
		   state of t, tlen may be larger than slen. */
		const wchar_t *Sp = S;
