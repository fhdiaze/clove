...
#define strtol(...) CALL3(strtol, "0", nullptr, 0, __VA_ARGS__)
#define strtoll(...) CALL3(strtoll, "0", nullptr, 0, __VA_ARGS__)
...
inline signed maxs(signed a, signed b) [[__unsequenced__]]
{
        return a < b ? b : a;
}

inline unsigned maxu(unsigned a, unsigned b) [[__unsequenced__]]
{
        return a < b ? b : a;
}

inline unsigned maxus(unsigned a, signed b) [[__unsequenced__]]
{
        return b < 0 ? a : maxu(a, b);
}

inline unsigned maxsu(signed a, unsigned b) [[__unsequenced__]]
{
        return a < 0 ? b : maxu(a, b);
}

inline signed long maxsl(signed long a, signed long b) [[__unsequenced__]]
{
        return a < b ? b : a;
}

inline unsigned long maxul(unsigned long a, unsigned long b) [[__unsequenced__]]
{
        return a < b ? b : a;
}

inline unsigned long maxusl(unsigned long a, signed long b) [[__unsequenced__]]
{
        return b < 0 ? a : maxul(a, b);
}

inline unsigned long maxsul(signed long a, unsigned long b) [[__unsequenced__]]
{
