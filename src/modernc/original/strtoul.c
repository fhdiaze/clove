...
unsigned long Strtoul_inner(const char s[static 1], size_t i, unsigned base)
{
	unsigned long ret = 0;
	while (s[i]) {
...
unsigned long Strtoul(const char s[static 1], unsigned base)
{
	...
	if (!base || base == 16 || base == 2) { /* use prefix */
		...
		case 1: // There is a 0 prefix
			...
			default: // There is a 0b or a 0B prefix
				if (!base || base == 2)
					base = 2;
				// If we are looking for another base, the b is
				// the end
				else
					return 0;
				break;
...
int main(int argc, char *argv[argc + 1])
{
	...
	for (unsigned j = 0; j < sizeof code / sizeof code[0]; ++j) {
		...
		puts("");
		if (code[j])
			printf("%u:\tULONG_MAX %s\n", code[j],
			       char36(ULONG_MAX, code[j], (char[256]){}));
	}
	return EXIT_SUCCESS;
}
